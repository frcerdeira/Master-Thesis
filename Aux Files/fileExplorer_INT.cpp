...
inline std::string eraseString(std::string fileName, std::string deleteThis){

    std::string newName;
    int pos = 0;

    pos = fileName.find(deleteThis);
...
void mergeFiles(std::vector<std::string>& fileNames, std::string fileName){
    ...
    nameNewFile = eraseString(fileName, "."); // file name provided by user to what we will do shit + file extension
...
int main()
{
    ...
    do{
        ...
        while(value <= inputEnd + 0.001){ // 0.001 to make sure there aren't any rounding errors
            ...
            valueAsString = std::to_string(value) + "/"; // I use the bar so I can know that the name stops here

            folderName = inputParameterName + "_=" + valueAsString;

            namesFiltered = filterNames(names, folderName); // filter files - specific folder

            folderName = eraseString(folderName, "/");
