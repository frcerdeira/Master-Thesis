...
int seed = 0, samples = 0, Iterations = 0, binSize = 0, frames = 0; // Statistics
...
int constructInteractTable(std::vector<double> &table){

    /**** Loop Variables ****/
    int chain = 0, bead = 0, bPc = 0, count = 0;
...
inline void evalForce(const int& t, const int& numOfPossibleBonds, const double& BoxLength, const double& HalfLength, const int& totalBeads,
                std::vector<double> &rx, std::vector<double> &ry,
                std::vector<double> &intraFx, std::vector<double> &intraFy,
                std::vector<double> &interFx, std::vector<double> &interFy, std::vector<double> table,
    ...
    /**** Loop Variables ****/
    int chain = 0, bead = 0, next = 0, nb = 0, doPot = 0;
    ...
    for(chain = 0; chain < chainNumber; chain++){
        for(bead = 0; bead < chainSize; bead++){
            ...
            if(table[bPc] != 0){
                ...
                for(next = bPc + 1; next < totalBeads; next++){

                    if(table[next] != 0 && table[next] != table[bPc]){
                    ...
                    } // end if used to check if next bead is of the interacting kind
                } // end for loop used to go over next interacting beads 
            } // end if to see if the current bead is of the interacting kind
        } // end for Bead
    } // end for Chain
...
/**** [FUNCTION] Evaluate Force [FUNCTION] ****/
inline void evalForce_temp(const double& BoxLength, const double& HalfLength, const int& totalBeads,
                std::vector<double> &rx_temp, std::vector<double> &ry_temp,
                std::vector<double> &intraFx_temp, std::vector<double> &intraFy_temp,
                std::vector<double> &interFx_temp, std::vector<double> &interFy_temp, std::vector<double> table,
                const double& rc2, const double& peak2, const double& s6, const double& s12, const double& b2){
...
inline void Cannes(std::ofstream& video, int t, int attractingBeads, int totalBeads, int numOfPossibleBonds, std::vector<int> James, std::vector<double> X, std::vector<double> Y){
    ...
    for(r = 0; r < totalBeads; r++){
        ...
        if(r%chainSize == 0 || r%chainSize == chainSize-1){
            atom = bond[var3]+2;
            var3++;
        }
        else{
...
int main(int argc, char const *argv[]){
    ...
    vector<double> table(totalBeads); 
    const int interactBeads = constructInteractTable(table);
    ...
    for(spl = 0; spl < samples; spl++){
        ...
        for(t = 0; t < Iterations; t++){
            ...
            /**** Evaluation of Force ***/
            evalForce(t, numOfPossibleBonds, BoxLength, HalfLength, totalBeads, rx, ry, intraFx, intraFy,
                     interFx, interFy, table, rc2, peak2, s6, s12, b2, bondGPSprev, formedBonds, brokenBonds, numberOfBonds);
            ...
            evalForce_temp(BoxLength, HalfLength, totalBeads, rx_temp, ry_temp, intraFx_temp, intraFy_temp,
                     interFx_temp, interFy_temp, table, rc2, peak2, s6, s12, b2);
    ...
    for(j = 0; j < arraySizeSimul; j++){ 
        ...
        file1 << numBondsVector[j]/samplesTbinSizeTparticleNumber << "\t" << tS << endl;
